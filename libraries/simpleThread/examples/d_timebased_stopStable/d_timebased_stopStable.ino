/* ******************************************************************** */
/* Example: static timebased threads with stable state on thread stop
/* ******************************************************************** */
/* Description: (german)
 *
 * Die Funktionsbeschreibung ist die gleiche wie in Beispiel (a)
 * Nur wird hier für Thread B ein Stabilder Zustand aufgerufen 
 * nachdem der Thread gestoppt wurde
 *
 *
 *
 * In diesem Beispiel werden zwei statische Threads angelegt. Beide 
 * besitzen eine feste Zeit. Thread A wird jede Sekunde aufgerufen
 * Thread B alle 50 Millisekunden. 
 * Zu beginn wird nur Thread A gestartet. In diesem wird ein Zähler bis
 * zum Wert 5 hochgezählt. Dann wird Thread B aus Thread A gestartet.
 * Thread B arbeitet mit dem gleichen Zähler. Im Setup von B wird der 
 * Zähler zurückgesetzt, anschließend Zählt der Thread B bis 3 und 
 * stoppt sich selbst. Dies geschiet in 150 ms, das heißt bevor Thread A
 * erneut zählen kann, hat Thread B seine Aufgabe erfüllt.
 */
 ...
 *
 */  
  simpleThread_new_timebased_dynamic (_sT_P1  , _sT_millis, 1000    , _sT_start  , Thread_A); 
  simpleThread_new_timebased_dynamic (_sT_P2  , _sT_millis, 50      , _sT_stop  , Thread_B); 
